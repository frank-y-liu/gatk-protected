...
Hidden states have two labels, $k \in \{1, \, \ldots K \}$ and $s \in \{0, \, 1 \}$, where $k$ selects the minor allele fraction $f_k$ and $s = 1$ represents a ``switch'' (memory loss) state.  We give each hidden state $f_k$ a weight $\pi_k$ for each $f_k$, so that the the transition probabilities for het sites a distance $d$ bases apart is
%
\begin{equation}
P(k,s \rightarrow k^\prime, s^\prime) = \left\{  \begin{array}{cc} e^{-d/D} \delta_{k, k^\prime} & s^\prime = 0 \\ (1-e^{-d/D}) \pi_{k^\prime} & s^\prime = 1     \end{array}  \right.
\end{equation}
%
We encourage sparsity by placing a symmetric Dirichlet prior with concentration parameter $\alpha$, $\pi | \alpha \sim {\rm Dir}(\alpha/K, \ldots \alpha /K)$, on $\pi$.  Letting $z_{iks}$ be a binary indicator for hidden state $(s,k)$ at het site $i$, the model likelihood is
\begin{equation}
P(\alpha)  \left( \frac{ \Gamma(\alpha) }{\Gamma(\alpha/K)^K} \prod_k \pi_k^{\alpha/K - 1} \right) \left(  \prod_{i,s,k} P(a_i, r_i | f_k)^{z_{isk}} \right) \left( \prod_{i,s,k,s^\prime, k^\prime} \left( e^{-d/D} \delta_{k, k^\prime} \delta_{s^\prime,0} +  (1 - e^{-d/D}) \pi_{k^\prime} \delta_{s^\prime,1} \right)^{z_{i+1,s^\prime, k^\prime}} \right)
\label{HMM_likelihood}
\end{equation}

Using the forward-backward algorithm, which is already in our code base, we can obtain exact posterior probabilities $E[z_{isk}]$ of each hidden state, which constitutes the E step of an EM approach to learning $\pi$, $f$, and $D$.  Taking the logarithm of Equation \ref{HMM_likelihood} we can read off the M step equations.  The M step for $D$ is
\begin{equation}
D = \argmax_D \sum_{i,k} \left( E[z_{i, s=0, k}] \ln e^{-d_i/D} +  E[z_{i, s=1, k}] \ln (1 - e^{-d_i/D}) \right)
\end{equation}
The M step for $f_k$ is
\begin{equation}
f_k = \argmax_f \sum_{i,s} E[z_{i,s,k}] \ln P(a_i, r_i | f)
\end{equation}

Due to the singularity of the Dirichlet prior we must use variational Bayes, rather than maximum likelihood, on $\pi$.  The M step posterior on $\pi$ is, due to conjugacy, another Dirichlet with pseudocounts derived from the likelihood.  By inspection, this posterior is
%
\begin{equation}
q(\pi) = {\rm Dir}(\pi | \alpha/K + N_1, \ldots \alpha/K + N_2), \, N_k \equiv \sum_{i} E[z_{i, s=1, k}]
\end{equation}
%
It is easily seen that the correct variational Bayes prescription for $\pi_k$ to be used in the E step for $z$ and the M step for $\alpha$ is the replacement $\pi_k \rightarrow \bar{\pi}_k \equiv \exp E_{q(\pi)}[\ln \pi_k]$. The standard analytic result for this is
\begin{equation}
E[\ln \pi_k] = \psi(\alpha/K + N_k) - \psi(\alpha + \sum_k N_k)
\end{equation}
where $\psi$ is the digamma function.

It is also seen by inspection that the effective value of $\alpha$ to be used in the M step for $\pi$ is the mean $\bar{\alpha}$ of the posterior $q(\alpha)$ (that is, the part of the likelihood containing $\alpha$ with the replacement $\pi_k \rightarrow \bar{\pi}_k$), which amounts to a simple 1-D quadrature.  Thus the M step amounts to $K + 1$ numerical optimizations (for $D$ and $\{ f_k \}$) of objectives containing $T$ terms each along with several very cheap operations.  The E step's time complexity is the cost $O(K^2 T)$ of the forward-backward algorithm.  We note that since the allele fraction model requires segmentation information, during EM iteration we should also perform segmentations via the Viterbi algorithm and relearn the parameters of the allele fraction model.  The Viterbi algorithm also has cost $O(K^2 T)$.
