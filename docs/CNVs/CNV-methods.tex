...
%
\begin{equation}
P(\vn_s, \vb, \vz | \vc, \vW, \vm, \vPsi, d ) = \prod_s {\rm N}(\vz_s | \vzero, \vI) {\rm N}(\vb_s | \vW \vz_s + \vm, \vPsi) {\rm N}(\vb_s | \vm_s, \vSigma_s)
\end{equation}
%
...
%
\begin{equation}
P(\vc, \vz | \vc, \vW, \vm, \vPsi, d, \vn) = \prod_s {\rm N}(\vz_s | \vzero, \vI) {\rm N}(\vm_s | \vW \vz_s + \vm, \vPsi + \vSigma_s)
\end{equation}
%
...
%
\begin{align}
\vG_s =& \left( \vI + \vW^T \vPsi_s W \right)^{-1} \\
E \left[ \vz_s \right] =& \vG_s W^T \vPsi_s (\vm_s - \vm) \\
...
%
\begin{align}
E \left[ \ln {\rm N}(\vm_s | \vW \vz_s + \vm, \vPsi + \vSigma_s) \right] =& -\frac{1}{2} \sum_{st} \left[ \ln \vPsi_{st}+ E[(\vW \vz_s + \vm - \vm_s)_t^2] (\vPsi_{st})^{-1} \right] 
\end{align}
which reduces to
\begin{equation}
 -\frac{1}{2} \sum_{st} \left[ \ln (\vPsi_t + \Sigma_{st}) + \left( \vW_t  E[\vz_s \vz_s^T] \vW_t^T + (\vm_t - \vm_{st})^2 +2 (\vm_t - \vm_{st}) \vW_t E[\vz_s] \right)\vPsi_{st}^{-1} \right]
\end{equation}
%
...
\begin{align}
\sum_s \frac{1}{\vPsi_t + \Sigma_{st}} - \frac{\left( \vW_t  E[\vz_s \vz_s^T] \vW_t^T + (\vm_t - \vm_{st})^2 +2 (\vm_t - \vm_{st}) \vW_t E[\vz_s] \right)}{(\vPsi_t + \Sigma_{st})^2} = 0,
\end{align}
which is straightforward to solve numerically.  Alternatively, we could approximate $\vSigma_s \approx \bar{\vSigma} \equiv \frac{1}{S} \sum_s \vSigma_s$, in which case we obtain
%
\begin{equation}
\vPsi_t + \bar{\vSigma}_t = \frac{1}{S} \sum_s \left( \vW_t  E[\vz_s \vz_s^T] \vW_t^T+ (\vm_t - \vm_{st})^2 +2 (\vm_t - \vm_{st})  \vW_t E[\vz_s] \right)
\end{equation}
%

The M step equation for the row $\vW_t$ is also routine and gives
%
\begin{equation}
\vW_t^T = \left( \sum_s \vPsi_{st}^{-1}  E[\vz_{s} \vz_{s}^T]  \right)^{-1} \sum_s \vPsi_{st}^{-1} ( \vm_{st} - \vm_t) E[\vz_{s}]
\end{equation}

We may automatically select the number of latent features to use via an automatic relevance determination (ARD) prior on $\vW$.  That is, we add a term
\begin{equation}
P(\vW) = \prod_k \left( \frac{\alpha_k}{2 \pi} \right)^{T/2} \exp \left( -\frac{1}{2} \alpha_k \sum_t W_{tk}^2  \right)
\end{equation}
The E step is unaffected by this addition and the only change to the M step is that in Equation \ref{M_step_W} we now have
\begin{equation}
\vM_t = {\rm Diag}(\alpha_1, \alpha_2 \ldots \alpha_K) + \sum_s \vPsi_{st}^{-1}  E[\vz_{s} \vz_{s}^T] 
\end{equation}
...
\end{equation}
The meaning of these parameters is that $\alpha_k \rightarrow \infty$ sets the $k$-th column of $\vW$ to zero, turning off the $k$-th latent feature.  Thus we can initially choose a liberal estimate of $K$ and the model will automatically become more parsimonious.
...
\begin{equation}
P({\rm data} | \vc) \propto \prod_t \exp \left[ -\frac{1}{2} \left(  (\vm_t - \vm_{st})^2 +2 (\vm_t - \vm_{st}) \vW_{t} E[\vz_{s}] \right)\vPsi_{st}^{-1} \right],
\end{equation}
