...
public final class Genome {
    private final TargetCollection<ReadCountRecord.SingletonRecord> targets;
     ...
     */
    public Genome(ReadCountCollection targets, final List<AllelicCount> snps, final String sampleName) {
        ...
        this.targets = new HashedListTargetCollection<>(targets.records().stream().map(ReadCountRecord::asSingleton).collect(Collectors.toList()));
     ...
     * @param normalizedCoverageFile    linear target-coverage file
     * @param snpFile       SNP-allele-count file
     * @param sampleName    name of the sample
     */
    public Genome(final File normalizedCoverageFile, final File snpFile, final String sampleName) {
        try {
            final ReadCountCollection poo = ReadCountCollectionUtils.parse(normalizedCoverageFile);
            targets = new HashedListTargetCollection<>(ReadCountCollectionUtils.parse(normalizedCoverageFile).records()
                    .stream().map(ReadCountRecord::asSingleton).collect(Collectors.toList()));
    ...
    public final TargetCollection<ReadCountRecord.SingletonRecord> getTargets() {  return targets; }
