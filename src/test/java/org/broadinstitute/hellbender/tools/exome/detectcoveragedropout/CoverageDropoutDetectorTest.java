...
final public class CoverageDropoutDetectorTest extends BaseTest {
    ...
    private Object[][] getUnivariateGaussianTargets(final double sigma) {
        ...
        final List<ReadCountRecord.SingletonRecord> targetList = new ArrayList<>();
        for (int i = 0; i < (numDataPoints - numEventPoints); i++){
            targetList.add(new ReadCountRecord.SingletonRecord(new Target("arbitrary_name", new SimpleInterval("chr1", 100 + 2*i, 101 + 2 * i)), n.sample()));
        }
        for (int i = (numDataPoints - numEventPoints); i < numDataPoints; i++){
            targetList.add(new ReadCountRecord.SingletonRecord(new Target("arbitrary_name", new SimpleInterval("chr1", 100 + 2 * i, 101 + 2 * i)), 0.5 + n.sample()));
        }

        HashedListTargetCollection<ReadCountRecord.SingletonRecord> targets = new HashedListTargetCollection<>(targetList);
    ...
    private Object[][] getUnivariateGaussianTargetsWithDropout(final double sigma, final double dropoutRate) {
        ...
        for (int i = 0; i < numDataPoints; i++){
            ...
            targetList.add(new ReadCountRecord.SingletonRecord(new Target("arbitrary_name", new SimpleInterval("chr1", 100 + 2*i, 101 + 2 * i)), coverage));
        }
    ...
    public void testNoError(final TargetCollection<ReadCountRecord.SingletonRecord> tc, final List<ModeledSegment> segments){
    ...
    public void testSmallLowVarianceDropout(final TargetCollection<ReadCountRecord.SingletonRecord> tc, final List<ModeledSegment> segments){
        CoverageDropoutDetector c = new CoverageDropoutDetector();
